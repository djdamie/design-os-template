...
import type { EditableFieldProps, ReferenceTrack, TeamMemberOption } from '@/../product/sections/project-canvas/types'
import { parseFlexibleDate, normalizeDateToISO } from '@/lib/utils'
...
export function EditableField({ field, teamMembers = [], onUpdate }: EditableFieldProps) {
  ...
  const handleSave = () => {
    const valueToSave = field.type === 'date' && typeof editValue === 'string'
      ? normalizeDateToISO(editValue)
      : editValue
        ...
        return (
          <div className="relative">
            <input
              ...
              type="date"
              value={normalizeDateToISO(String(editValue || ''))}
              onChange={(e) => setEditValue(e.target.value)}
              onKeyDown={handleKeyDown}
  ...
  const renderDisplayValue = () => {
    ...
    if (field.type === 'date' && field.value) {
      const date = parseFlexibleDate(String(field.value))
      if (!date) {
        return (
          <span className="text-sm text-zinc-900 dark:text-zinc-100 font-['DM_Sans']">
            {String(field.value)}
          </span>
        )
      }
