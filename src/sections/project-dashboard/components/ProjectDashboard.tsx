...
import type {
  ...
  ProjectType,
  SortField,
} from '@/../product/sections/project-dashboard/types'
  ...
  const filteredProjects = useMemo(() => {
    ...
    if (filters.dateRange.start || filters.dateRange.end) {
      result = result.filter((p) => {
        ...
        const deadline = parseFlexibleDate(p.deadline)
        if (!deadline) return false
        const start = filters.dateRange.start ? parseFlexibleDate(filters.dateRange.start) : null
        const end = filters.dateRange.end ? parseFlexibleDate(filters.dateRange.end) : null
    ...
    result.sort((a, b) => {
      ...
      switch (sort.field) {
        ...
        case 'deadline':
          ...
          else comparison = (parseFlexibleDate(a.deadline)?.getTime() ?? 0) - (parseFlexibleDate(b.deadline)?.getTime() ?? 0)
          break
        case 'createdAt':
          comparison = (parseFlexibleDate(a.createdAt)?.getTime() ?? 0) - (parseFlexibleDate(b.createdAt)?.getTime() ?? 0)
          break
        case 'updatedAt':
        default:
          comparison = (parseFlexibleDate(a.updatedAt)?.getTime() ?? 0) - (parseFlexibleDate(b.updatedAt)?.getTime() ?? 0)
          break
      }
