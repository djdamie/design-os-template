...
import type { WorkflowStep, TeamRole } from '@/../product/sections/workflow-tracker/types'
      ...
      {isExpanded && (
        <div className="px-3 pb-3 border-t border-zinc-100 dark:border-zinc-700/50 pt-3 space-y-3">
          ...
          <div className="flex gap-4 text-xs text-zinc-500 dark:text-zinc-400">
            {step.startedAt && (
              <span>Started: {(parseFlexibleDate(step.startedAt) ?? new Date(step.startedAt)).toLocaleDateString()}</span>
            )}
            {step.completedAt && (
              <span>Completed: {(parseFlexibleDate(step.completedAt) ?? new Date(step.completedAt)).toLocaleDateString()}</span>
            )}
            {step.dueDate && !step.completedAt && (
              <span className="text-amber-600 dark:text-amber-400">
                Due: {(parseFlexibleDate(step.dueDate!) ?? new Date(step.dueDate!)).toLocaleDateString()}
              </span>
            )}
          </div>
